...
const StyledActionButtonGroup = styled(ActionButtonGroup)<{ backgroundurl?: string }>(({ theme, backgroundurl }) => ({
...
const FileSelectedActions = forwardRef(({ targets }: FileSelectedActionsProps, ref: React.Ref<HTMLElement>) => {
  ...
  return (
    <Box ref={ref} sx={{ height: "100%" }}>
      <Stack direction={"row"} spacing={1} sx={{ height: "100%" }}>
        <StyledActionButtonGroup backgroundurl={backgroundUrl} variant="outlined">
          <ActionButton
            onClick={() =>
              dispatch(
                clearSelected({
