...
export function useAreaSelection(container: React.RefObject<HTMLElement>, explorerIndex: number, enabled: boolean) {
  ...
  const updateCandidate = useCallback((containerElement: HTMLElement) => {
    ...
    console.log("AreaSelection: updateCandidate elements found", elements.length);
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent<HTMLElement>) => {
      console.log("AreaSelection: mouseMove", { mouseMoving: mouseMoving.current, enabled });
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent<HTMLElement>) => {
      console.log("AreaSelection: mouseDown", { button: e.button, enabled, target: e.target });
      e.preventDefault();
      ...
      if (!containerElement || !containerElement.contains(e.target as HTMLElement)) {
        console.log("AreaSelection: mouseDown target not inside container", { containerElement, target: e.target });
        return;
      }
  ...
  useEffect(() => {
    ...
    if (mouseDown) {
      ...
      if (document.body.contains(selectionBoxElement)) {
        document.body.removeChild(selectionBoxElement);
      }
    }
  }, [mouseDown]);
  ...
  // Cleanup RAF and DOM elements on unmount
  useEffect(() => {
    return () => {
      if (rafIdRef.current !== null) {
        cancelAnimationFrame(rafIdRef.current);
      ...
      const selectionBoxElement = boxRef.current;
      if (selectionBoxElement && document.body.contains(selectionBoxElement)) {
        document.body.removeChild(selectionBoxElement);
      }
    };
  }, []);
